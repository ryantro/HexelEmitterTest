...
class Application:
    def __init__(self, master):
        ...
        self.arduino = instruments.MuController()
...
# BEGINNING OF WATER FLOW TAB IMPLEMENTATION # ------------------------------------------------------------------------
    ...
    def get_data(self):
        try:
            # self.ser = instruments.MuController()
            # self.ser.isOpen()
            # time.sleep(2)
            # self.ser.flush()
        ...
        while True:
            text = self.arduino.ser.readline().decode()
            data = text.split()
            value = data[0]
            value2 = data[1]
    ...
    def purge_system(self):
        self.threshold = str("0.00")
        if self.FlowValue.get() == self.threshold:
            self.arduino.purge()
        else:
            self.purge_check()
        return
    ...
    def flow_warning(self):
        """
        INFORMING USER THAT WATER FLOW HAS STOPPED.
        """
        if tk.messagebox.showinfo("Warning", "Water flow has stopped, you must have at least 0.5 Lpm to run a test."):
            return
    ...
    def zero_flow(self):
        ...
        INFORMING USER THAT WATER FLOW HAS STOPPED.
        """
        if self.FlowValue.get() == ("0.00"):
            self.mprint("water flow has stopped")
            # time.sleep(10)
        else:
            pass
